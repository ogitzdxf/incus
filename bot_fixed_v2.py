...
def init_db():
    ...
    if 'os_version' not in columns:
        cur.execute("ALTER TABLE vps ADD COLUMN os_version TEXT DEFAULT 'images:ubuntu/22.04'")
...
def get_vps_data() -> Dict[str, List[Dict[str, Any]]]:
    ...
    for row in rows:
        ...
        vps['os_version'] = vps.get('os_version', 'images:ubuntu/22.04')
...
def save_vps_data():
    ...
    for user_id, vps_list in vps_data.items():
        for vps in vps_list:
            ...
            os_ver = vps.get('os_version', 'images:ubuntu/22.04')
...
async def create_port_forward(user_id: str, container: str, vps_port: int) -> Optional[int]:
    ...
    if not host_port:
        return None
    try:
        await execute_incus(f"incus config device add {container} tcp_proxy_{host_port} proxy listen=tcp:0.0.0.0:{host_port} connect=tcp:127.0.0.1:{vps_port}")
        await execute_incus(f"incus config device add {container} udp_proxy_{host_port} proxy listen=udp:0.0.0.0:{host_port} connect=udp:127.0.0.1:{vps_port}")
        conn = get_db()
        cur = conn.cursor()
...
async def remove_port_forward(forward_id: int, is_admin: bool = False) -> tuple[bool, Optional[str]]:
    ...
    try:
        await execute_incus(f"incus config device remove {container} tcp_proxy_{host_port}")
        await execute_incus(f"incus config device remove {container} udp_proxy_{host_port}")
...
async def apply_lxc_config(container_name):
    try:
        await execute_incus(f"incus config set {container_name} security.nesting true")
        await execute_incus(f"incus config set {container_name} security.privileged true")
        await execute_incus(f"incus config set {container_name} security.syscalls.intercept.mknod true")
        await execute_incus(f"incus config set {container_name} security.syscalls.intercept.setxattr true")
        
        # Only add FUSE device if it exists on the host to prevent startup failures
        try:
            # Check if /dev/fuse exists on the host
            check_fuse = await execute_incus("ls /dev/fuse")
            await execute_incus(f"incus config device add {container_name} fuse unix-char path=/dev/fuse")
        except Exception as e:
            if "already exists" not in str(e).lower() and "No such file or directory" not in str(e).lower():
                logger.warning(f"Could not add FUSE device to {container_name}: {e}")
...
lxc.cap.drop =
"""
        await execute_incus(f"incus config set {container_name} raw.lxc '{raw_lxc_config}'")
...
async def apply_internal_permissions(container_name):
    try:
        ...
        for cmd in commands:
            try:
                await execute_incus(f"incus exec {container_name} -- bash -c \"{cmd}\"")
            except Exception:
                continue
...
async def get_container_logs(container_name, lines=50):
    try:
        proc = await asyncio.create_subprocess_exec(
            "incus", "exec", container_name, "--", "journalctl", "-n", str(lines), "--no-pager",
            stdout=asyncio.subprocess.PIPE,
            stderr=asyncio.subprocess.PIPE
        )
        stdout, _ = await proc.communicate()
    ...
    try:
        ...
        if b"NOT_FOUND" in cout:
            ...
            await asyncio.create_subprocess_exec(
                "incus", "exec", container_name, "--", "bash", "-c", install_cmd,
                stdout=asyncio.subprocess.PIPE,
                stderr=asyncio.subprocess.PIPE
            )
            await asyncio.sleep(5)
        ...
        await asyncio.create_subprocess_exec(
            "incus", "exec", container_name, "--", "bash", "-c", run_cmd
        )
...
async def run_panel_installer(container_name: str, choice: str,
    ...
    try:
        ...
        dep_proc = await asyncio.create_subprocess_exec(
            "incus", "exec", container_name, "--", "bash", "-c", dep_cmd,
            stdout=asyncio.subprocess.PIPE,
            stderr=asyncio.subprocess.PIPE
        )
        ...
        dl_proc = await asyncio.create_subprocess_exec(
            "incus", "exec", container_name, "--", "bash", "-c", dl_cmd,
            stdout=asyncio.subprocess.PIPE,
            stderr=asyncio.subprocess.PIPE
        )
        try:
...
async def list_user_vps(ctx):
    ...
    for i, vps in enumerate(vps_list, 1):
        ...
        vps_info += f"**OS:** {vps.get('os_version', 'images:ubuntu/22.04')}\n"
...
class ManageView(discord.ui.View):
    ...
    async def create_vps_embed(self, index):
        ...
        resource_info += f"**OS:** {vps.get('os_version', 'images:ubuntu/22.04')}\n"
        ...
        if action == 'start':
            ...
            try:
                ...
                if check_proc.returncode != 0:
                    ...
                    await execute_incus(f"incus exec {container_name} -- apt-get update -y")
                    await execute_incus(f"incus exec {container_name} -- apt-get install tmate -y")
            ...
            try:
                ...
                await execute_incus(f"incus exec {container_name} -- bash -c 'apt-get update -y'", timeout=60)
                await execute_incus(f"incus exec {container_name} -- bash -c 'apt-get install curl -y'", timeout=60)
                
                # Install and run SSHX
                await execute_incus(f"incus exec {container_name} -- bash -c 'curl -sSf https://sshx.io/get | sh'", timeout=30)
                ...
                sshx_proc = await asyncio.create_subprocess_exec(
                    "incus", "exec", container_name, "--", "bash", "-c", "sshx | grep -o 'https://sshx.io/[a-zA-Z0-9]*' | head -1",
                    stdout=asyncio.subprocess.PIPE,
                    stderr=asyncio.subprocess.PIPE
                )
...
async def vps_info(ctx, container_name: str):
    ...
    resources += f"**OS:** {found_vps.get('os_version', 'images:ubuntu/22.04')}"
...
async def clone_vps(ctx, container_name: str, new_name: str = None):
    ...
    try:
        await execute_incus(f"incus copy {container_name} {new_name}")
        embed = discord.Embed(
            title="✅ VPS Cloned",
...
async def create_snapshot(ctx, container_name: str, snap_name: str = None):
    ...
    try:
        await execute_incus(f"incus snapshot {container_name} {snap_name}")
        embed = discord.Embed(
...
async def restore_backup(ctx, container_name: str, snap_name: str):
    ...
    try:
        await execute_incus(f"incus restore {container_name} {snap_name}")
        embed = discord.Embed(
...
class OSSelectView(discord.ui.View):
    ...
    async def create_vps(self, interaction: discord.Interaction, os_version: str):
        ...
        try:
            await execute_incus(f"incus init {os_version} {container_name} -s {DEFAULT_STORAGE_POOL}")
            await execute_incus(f"incus config set {container_name} limits.memory {ram_mb}MB")
            await execute_incus(f"incus config set {container_name} limits.cpu {self.cpu}")
            await execute_incus(f"incus config device set {container_name} root size={self.disk}GB")
...
async def add_resources(ctx, container_name: str, ram: int = None, cpu: int = None, disk: int = None):
    ...
    if was_running:
        ...
        try:
            await execute_incus(f"incus stop {container_name}")
    ...
    try:
        ...
        if cpu is not None and cpu > 0:
            new_cpu += cpu
            await execute_incus(f"incus config set {container_name} limits.cpu {new_cpu}")
            changes.append(f"CPU: +{cpu} cores (New total: {new_cpu} cores)")
        
        if disk is not None and disk > 0:
            new_disk_gb += disk
            await execute_incus(f"incus config device set {container_name} root size={new_disk_gb}GB")
...
async def stop_all_vps(ctx):
    ...
    class ConfirmView(discord.ui.View):
        ...
        async def confirm(self, interaction: discord.Interaction, item: discord.ui.Button):
            await interaction.response.defer()
            try:
                await execute_incus("incus stop --all --force")
...
async def migrate_vps(ctx, container_name: str, pool: str):
    ...
    try:
        await execute_incus(f"incus move {container_name} -s {pool}")
        embed = discord.Embed(
            title="✅ VPS Migrated",
...
async def vps_network(ctx, container_name: str, action: str, value: str = None):
    ...
    try:
        if action == 'list':
            proc = await asyncio.create_subprocess_exec(
                "incus", "exec", container_name, "--", "ip", "addr",
                stdout=asyncio.subprocess.PIPE,
                stderr=asyncio.subprocess.PIPE
            )
            stdout, stderr = await proc.communicate()
            ...
            await execute_incus(f"incus config device set {container_name} eth0 limits.egress {value}")
            await execute_incus(f"incus config device set {container_name} eth0 limits.ingress {value}")
            embed = discord.Embed(
...
async def lxc_list(ctx):
    ...
    try:
        result = await execute_incus("incus list")
        embed = discord.Embed(
...
async def confirm_purge(ctx, user_id: str):
    ...
    for vps in vps_data[user_id][:]:
        try:
            await execute_incus(f"incus delete {vps['container_name']} --force")
            deleted_count += 1
        except:
            pass
    ...
    def update_embed(self):
        ...
        if self.current_category == "user":
            ...
            commands = [
                ...
                f"**`{PREFIX}incus-list`** - List all LXC containers (Admin)"
            ]
            total = 9
